...
class BitMonitor(Monitor):
    ...
    def __init__(self, name, signal, clock, reset,callback=None, event=None):
        self.name = name
        self.signal = signal
        self.clock = clock
        self.reset = reset
    ...
    def _monitor_recv(self):
        ...
        resetedge = Edge(self.reset)
        ro = ReadOnly()

        # wait for X to disappear
        yield Timer(1)

        while True:
            # Capture signal at rising edge of clock or at the edge of a reset
            yield [clkedge,resetedge]
...
class DFF_TB(object):
    def __init__(self, dut, init_val):
        ...
        self.output_mon = BitMonitor("output",
                                     signal=dut.q,
                                     clock=dut.c,
                                     reset=dut.rstn
        )
    ...
    def model(self, transaction):
        ...
        if not self.stopped:
            if self.dut.rstn == 0:
                self.expected_output.append(BinaryValue(0))
            else:
...
def run_test(dut):
    ...
    cocotb.fork(reset_gen(dut.rstn))
